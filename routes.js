...
function ensureClientAuth(req, res, next) {
    req.headers['authorization'] = 'Basic ' + req.body.client_id + ':' + req.body.client_secret;
    req._passport.authenticate('basic', {session: false}, next);
    //req.app.db.models.Client.findOne({ id: req.body.client_id }, function (err, client) {
    //  if (err) { return callback(err); }
    //
    //  // No client found with that id or bad password
    //  if (!client || client.secret !== req.body.client_secret) { return callback(null, false); }
    //
    //  // Success
...
function ensureTokenAuth(req, res, callback) {
    var token;
    ...
    req.app.db.models.Token.findOne({value: accessToken}, function (err, token) {
        if (err) {
            return callback(err);
        }
        ...
        req.app.db.models.User.findOne({_id: token.userId}, function (err, user) {
            if (err) {
                return callback(err);
            }
...
exports = module.exports = function (app, passport) {
    ...
    app.get('/signup/khbar4u/', passport.authenticate('khbar4u', {
        callbackURL: '/signup/khbar4u/callback/',
        scope: ['profile email']
    }));
    app.get('/signup/khbar4u/callback/', require('./views/signup/index').signupKhbar4u);
    ...
    app.get('/account/settings/khbar4u/', passport.authenticate('khbar4u', {
        callbackURL: '/account/settings/khbar4u/callback/',
        scope: ['profile email']
    }));
    app.get('/account/settings/khbar4u/callback/', require('./views/account/settings/index').connectKhbar4u);
    app.get('/account/settings/khbar4u/disconnect/', require('./views/account/settings/index').disconnectKhbar4u);

    // Create endpoint handlers for /clients
    app.post('/clients/', ensureAuthenticated, require('./views/client/index').postClients);
    app.get('/clients/', ensureAuthenticated, require('./views/client/index').getClients);

    // Create endpoint handlers for oauth2 authorize
    //app.all('/oauth2/authorize*',ensureAuthenticated);
    app.all('/oauth2/authorize*', ensureAuthenticated);
    app.get('/oauth2/authorize/', app.oauth2server.endpoints.authorization);
    app.post('/oauth2/authorize/', app.oauth2server.endpoints.decision);
    // Create endpoint handlers for oauth2 token
    app.post('/oauth2/token', passport.authenticate('client'), app.oauth2server.endpoints.token);
    app.all('/oauth2/userinfo', passport.authenticate('token', {session: false}), require('./views/admin/users/index').userInfo);
