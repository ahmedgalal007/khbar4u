...
exports.connectKhbar4u= function(req, res, next){
  req._passport.instance.authenticate('khbar4u', { callbackURL: '/account/settings/khbar4u/callback/' }, function(err, user, info) {
    if (!info || !info.profile) {
      return res.redirect('/account/settings/');
    }

    req.app.db.models.User.findOne({ 'khbar4u.id': info.profile.id, _id: { $ne: req.user.id } }, function(err, user) {
      if (err) {
        return next(err);
      }
      ...
      if (user) {
        ...
        req.app.db.models.User.findByIdAndUpdate(req.user.id, { 'khbar4u.id': info.profile.id }, function(err, user) {
          if (err) {
            return next(err);
          }
...
exports.disconnectKhbar4u = function(req, res, next){
  req.app.db.models.User.findByIdAndUpdate(req.user.id, { khbar4u: { id: undefined } }, function(err, user) {
    if (err) {
      return next(err);
    }
