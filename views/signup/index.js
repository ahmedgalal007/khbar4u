...
exports.init = function(req, res){
  if (req.isAuthenticated()) {
    ...
    res.render('signup/index', {
      ...
      oauthKhbar4u: !!req.app.config.oauth.khbar4u.key
    });
  }
};
...
exports.signupKhbar4u = function(req, res, next) {
  req._passport.instance.authenticate('khbar4u', { callbackURL: '/signup/khbar4u/callback/' }, function(err, user, info) {
    if (!info || !info.profile) {
      return res.redirect('/signup/');
    }

    req.app.db.models.User.findOne({ 'khbar4u.id': info.profile.id }, function(err, user) {
      if (err) {
        return next(err);
      }
      if (!user) {
        ...
        res.render('signup/index', {
          oauthMessage: 'We found a user linked to your khbar4u account.',
          oauthTwitter: !!req.app.config.oauth.twitter.key,
          oauthGitHub: !!req.app.config.oauth.github.key,
          oauthFacebook: !!req.app.config.oauth.facebook.key,
          oauthGoogle: !!req.app.config.oauth.google.key,
